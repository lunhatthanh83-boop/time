...
LANGUAGES = {
    'vi': {
        ...
        'send_time': "Nhập số lượng thời gian:",
        ...
        'select_time_unit': "Chọn đơn vị thời gian:",
        'unit_days': "📅 Ngày",
        'unit_hours': "⏰ Giờ",
        'unit_minutes': "⏱️ Phút",
    },
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if callback_data.startswith('settime_mb_'):
        ...
        # Show time unit selection
        keyboard = [
            [InlineKeyboardButton(t(uid, 'unit_days'), callback_data=f'settime_unit_{chat_id}_{member_id}_days')],
            [InlineKeyboardButton(t(uid, 'unit_hours'), callback_data=f'settime_unit_{chat_id}_{member_id}_hours')],
            [InlineKeyboardButton(t(uid, 'unit_minutes'), callback_data=f'settime_unit_{chat_id}_{member_id}_minutes')],
        ...
        await query.edit_message_text(
            ...
            f"ID: {member_id}\n\n"
            f"{t(uid, 'select_time_unit')}",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    
    if callback_data.startswith('settime_unit_'):
        parts = callback_data.split('_')
        chat_id = int(parts[2])
        member_id = int(parts[3])
        time_unit = parts[4]
        ...
        await query.edit_message_text(
            ...
            f"Tên: {member_info['full_name']}\n"
            f"ID: {member_id}\n"
            f"Đơn vị: {unit_display[time_unit]}\n\n"
            f"{t(uid, 'send_time')}",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton(t(uid, 'cancel'), callback_data='main_menu')
            ]])
    ...
    if callback_data.startswith('extend_mb_'):
        ...
        # Show time unit selection
        keyboard = [
            [InlineKeyboardButton(t(uid, 'unit_days'), callback_data=f'extend_unit_{chat_id}_{member_id}_days')],
            [InlineKeyboardButton(t(uid, 'unit_hours'), callback_data=f'extend_unit_{chat_id}_{member_id}_hours')],
            [InlineKeyboardButton(t(uid, 'unit_minutes'), callback_data=f'extend_unit_{chat_id}_{member_id}_minutes')],
            [InlineKeyboardButton(t(uid, 'cancel'), callback_data='main_menu')]
        ]
    ...
    if callback_data.startswith('extend_unit_'):
        parts = callback_data.split('_')
        chat_id = int(parts[2])
        member_id = int(parts[3])
        time_unit = parts[4]
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    time_unit = state.get('time_unit', 'days')  # Default to days for backward compatibility
    ...
    try:
        if action in ['set_time', 'extend_time']:
            ...
            # Convert to timedelta based on time_unit
            if time_unit == 'days':
                time_delta = timedelta(days=time_value)
                unit_display = f"{time_value} ngày"
            elif time_unit == 'hours':
                time_delta = timedelta(hours=time_value)
                unit_display = f"{time_value} giờ"
            elif time_unit == 'minutes':
                time_delta = timedelta(minutes=time_value)
                unit_display = f"{time_value} phút"
            else:
                time_delta = timedelta(days=time_value)
                unit_display = f"{time_value} ngày"
            ...
            if action == 'set_time':
                ...
                if not current_expire:
                    # If no current rental, set new one from now
                    new_expire = datetime.now() + time_delta
                else:
                    # Extend from current expiry
                    new_expire = current_expire + time_delta
                ...
                await update.message.reply_text(
                    ...
                    f"ID: {member_id}\n"
                    f"Kênh: {channel_name}\n"
                    f"Đã gia hạn thêm: {unit_display}\n"
        ...
        await update.message.reply_text(
            f"{t(uid, 'error')} Vui lòng nhập số hợp lệ!"
        )
    except Exception as e:
        await update.message.reply_text(
            f"{t(uid, 'error')} {str(e)}"
...
def main():
    ...
    INITIAL_ADMIN_ID = 6557052839  # Set your admin ID here (MUST BE INTEGER, NOT STRING!)
    
    # Always add initial admin if specified and not already in list
    if INITIAL_ADMIN_ID and INITIAL_ADMIN_ID not in rental_data.admins:
