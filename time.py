...
LANGUAGES = {
    'vi': {
        ...
        'enter_user_id': "Nhập User ID:",
        'enter_days': "Nhập số ngày:",
        'send_format': "Gửi theo định dạng: [user_id] [days]\nVí dụ: 123456789 30",
        'send_user_id': "Gửi User ID cần thao tác:",
        'send_days_extend': "Gửi số ngày cần gia hạn:",
    },
    'en': {
        ...
        'send_format': "Send in format: [user_id] [days]\nExample: 123456789 30",
        'send_user_id': "Send User ID to manage:",
        'send_days_extend': "Send number of days to extend:",
    }
}
...
async def settime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 3:
        await update.message.reply_text(
            "Format: /settime [channel_id] [user_id] [days]\n"
            "Example: /settime -1001234567890 123456789 30"
        )
        return
    ...
    try:
        ...
        if chat_id not in rental_data.bot_channels:
            await update.message.reply_text("Channel ID not found!")
            return
        
        expire_time = datetime.now() + timedelta(days=days)
        rental_data.set_rental(chat_id, target_user_id, expire_time)
        
        channel_name = rental_data.bot_channels[chat_id]
        await update.message.reply_text(
            f"{t(uid, 'success')}\n\n"
            f"User: {target_user_id}\n"
            f"Channel: {channel_name}\n"
            f"Days: {days}\n"
            f"Expire: {expire_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        
    except ValueError:
        await update.message.reply_text("Invalid input! Please check format.")
    except Exception as e:
        await update.message.reply_text(f"{t(uid, 'error')} {str(e)}")
...
async def removetime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text(
            "Format: /removetime [channel_id] [user_id]\n"
            "Example: /removetime -1001234567890 123456789"
        )
        return
    ...
    try:
        ...
        rental_data.remove_rental(chat_id, target_user_id)
        ...
        await update.message.reply_text(f"{t(uid, 'success')} Removed rental and kicked user.")
...
async def extendtime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 3:
        await update.message.reply_text(
            "Format: /extendtime [channel_id] [user_id] [days]\n"
            "Example: /extendtime -1001234567890 123456789 7"
        )
        return
    ...
    try:
        ...
        current_expire = rental_data.get_rental(chat_id, target_user_id)
        if not current_expire:
            await update.message.reply_text("User has no active rental!")
            return
        
        new_expire = current_expire + timedelta(days=days)
        rental_data.set_rental(chat_id, target_user_id, new_expire)
        
        await update.message.reply_text(
            f"{t(uid, 'success')}\n\n"
            f"Extended by {days} days\n"
            f"New expire: {new_expire.strftime('%Y-%m-%d %H:%M:%S')}"
        )
...
async def memberinfo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text("Format: /memberinfo [channel_id] [user_id]")
        return

    try:
        chat_id = int(context.args[0])
        member_user_id = int(context.args[1])
        
        member_info = rental_data.get_member_info(chat_id, member_user_id)
        if not member_info:
        ...
        info_text = (
            f"MEMBER INFO\n\n"
            f"Name: {member_info['full_name']}\n"
            f"Username: {member_info['username']}\n"
            f"ID: {member_user_id}\n"
            f"Status: {member_info['status']}\n"
            f"Join Time: {member_info['join_time']}"
        )
...
async def searchmember(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("Format: /searchmember [channel_id] [search_term]")
        return
    ...
    try:
        ...
        for member_id, info in members.items():
            if (search_term in info['full_name'].lower() or 
                search_term in info['username'].lower() or 
                search_term in str(member_id)):
                results.append(f"• {info['full_name']} ({info['username']}) - ID: {member_id}")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if callback_data.startswith('settime_ch_'):
        ...
        user_states[uid] = {'action': 'set_time', 'chat_id': chat_id}
        
        channel_name = rental_data.bot_channels.get(chat_id, "Unknown")
        await query.edit_message_text(
            f"{t(uid, 'select_channel')} {channel_name}\n\n{t(uid, 'send_format')}",
            reply_markup=InlineKeyboardMarkup([[
    ...
    if callback_data.startswith('extend_ch_'):
        chat_id = int(callback_data.split('_')[2])
        user_states[uid] = {'action': 'extend_time', 'chat_id': chat_id}
    ...
    if callback_data.startswith('remove_ch_'):
        chat_id = int(callback_data.split('_')[2])
        user_states[uid] = {'action': 'remove_time', 'chat_id': chat_id}
    ...
    if callback_data == 'member_menu':
        ...
        keyboard = [
            [InlineKeyboardButton(t(uid, 'member_info'), callback_data='member_info')],
            [InlineKeyboardButton(t(uid, 'search_member'), callback_data='search_member')],
            [InlineKeyboardButton(t(uid, 'export_members'), callback_data='export_members')],
            [InlineKeyboardButton(t(uid, 'back_to_main'), callback_data='main_menu')]
        ]
        await query.edit_message_text(
            "MEMBER MANAGEMENT\n\nSelect function:",
    ...
    if callback_data in ['member_info', 'search_member', 'export_members', 
        ...
        function_map = {
            'member_info': 'Use: /memberinfo [channel_id] [user_id]',
            'search_member': 'Use: /searchmember [channel_id] [search_term]',
            'export_members': 'Use: /exportmembers [channel_id]',
            'add_admin': 'Use: /addadmin [user_id]',
            'remove_admin': 'Use: /removeadmin [user_id]'
        }
        
        text = function_map.get(callback_data, 'Function in development')
        
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(t(uid, 'back_to_menu'), callback_data='member_menu' if callback_data in ['member_info', 'search_member', 'export_members'] else 'admin_menu')
        ]])
    ...
    try:
        if action == 'set_time':
            ...
            await update.message.reply_text(
                ...
                f"User: {target_user_id}\n"
                f"Channel: {channel_name}\n"
                f"Removed rental and kicked user.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(t(uid, 'back_to_main'), callback_data='main_menu')
                ]])
            )
        ...
        await update.message.reply_text(
            f"{t(uid, 'error')} Invalid input format!"
        )
    except Exception as e:
        await update.message.reply_text(
            f"{t(uid, 'error')} {str(e)}"
